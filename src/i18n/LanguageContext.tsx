...
export function LanguageProvider({ children }: { children: ReactNode }) {
  ...
  const formatDate = (date: Date): string => {
    ...
    const weekdays = getNestedValue(translations[language], 'clock.weekdays') as unknown as string[];
    const weekday = weekdays[weekdayIndex];
    const template = getNestedValue(translations[language], 'clock.dateFormat');
    
    if (language === 'en') {
      const months = getNestedValue(translations[language], 'clock.months') as unknown as string[];
      return template
  ...
  return (
    <LanguageContext.Provider value={{ language, setLanguage, t, formatWeather, formatDate, getPageTitle }}>
      {children}
