...
export async function GET(request: Request) {
  try {
    ...
    // 获取地理位置名称 (逆地理编码)
    let locationName = weatherData.resolvedAddress || getDefaultLocationName(language);
    ...
    try {
      ...
      console.warn('Geocoding failed, using weather API address:', geocodeError);
    ...
    const formattedWeather = {
      ...
      windDirection: getLocalizedWindDirection(currentConditions.winddir, language),
...
// 格式化位置名称
function formatLocationName(city: string, countryName: string, language: string): string {
  if (language === 'zh' || language === 'zh-TW') {
    // 中文：如果是中国，只显示城市名，不显示国家名
    return (countryName === 'China' || countryName === '中国' || countryName === '中华人民共和国') ? 
...
function getLocalizedWindDirection(degrees: number, language: string): string {
  const directionMaps: { [key: string]: string[] } = {
    ...
    'en': [
      'North Wind', 'Northeast Wind', 'East Wind', 'Southeast Wind',
...
// 将英文天气条件转换为中文 (保持向后兼容)
function getChineseCondition(condition: string): string {
  return getLocalizedCondition(condition, 'zh');
}

// 将风向角度转换为中文方向 (保持向后兼容)
function getChineseWindDirection(degrees: number): string {
